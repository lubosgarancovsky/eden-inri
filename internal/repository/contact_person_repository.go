...
func (r *ContactPersonRepository) FindAll(userID uuid.UUID, lq *list.ListingQuery) ([]model.ContactPerson, int64, error) {
	query := r.db.Model(&model.ContactPerson{}).Where("user_id = ?", userID)
	if lq.Filter != nil {
...
func (r *ContactPersonRepository) Update(cp *model.ContactPerson) (*model.ContactPerson, error) {
	result := r.db.Clauses(clause.Returning{}).Where("id = ?", cp.ID).Updates(&cp)
	if result.Error != nil {
...
func (r *ContactPersonRepository) Delete(id uuid.UUID) error {
	result := r.db.Clauses(clause.Returning{}).Where("id = ?", id).Delete(model.ContactPerson{})
	if result.Error != nil {
