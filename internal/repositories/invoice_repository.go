...
func (r *InvoiceRepository) FindAll(ctx context.Context, userID uuid.UUID, lq *list.ListingQuery) (*[]models.Invoice, int64, error) {
	query := r.db.
		WithContext(ctx).
	...
	items, total, err := helpers.List[models.Invoice](query, lq)
	if err != nil {
		return nil, 0, err
	}
...
func (r *InvoiceRepository) FindByID(ctx context.Context, userID, invoiceID uuid.UUID) (*models.Invoice, error) {
...
func (r *InvoiceRepository) Insert(ctx context.Context, inv *models.Invoice) (*models.Invoice, error) {
	if err := r.db.
		WithContext(ctx).
...
func (r *InvoiceRepository) Update(ctx context.Context, inv *models.Invoice) (*models.Invoice, error) {
	result := r.db.
		WithContext(ctx).
		Model(models.Invoice{}).
		Clauses(clause.Returning{}).
...
func (r *InvoiceRepository) MonthlyRevenue(ctx context.Context, userID uuid.UUID, from, to time.Time) ([]models.RevenueGraphPoint, error) {
	...
	err := r.db.WithContext(ctx).
		Table((&models.Invoice{}).TableName()).
	...
	// Map to model points
	points := make([]models.RevenueGraphPoint, 0, len(rows))
	for _, r := range rows {
