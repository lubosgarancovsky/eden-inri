...
type InvoiceService struct {
	r                 *repositories.InvoiceRepository
	attachmentService *AttachmentService
	ModelName         string
}

func NewInvoiceService(r *repositories.InvoiceRepository, attachmentService *AttachmentService) *InvoiceService {
	return &InvoiceService{r: r, attachmentService: attachmentService, ModelName: "invoice"}
}

func (s *InvoiceService) FindAll(ctx context.Context, userID uuid.UUID, lq *list.ListingQuery) (*[]models.Invoice, int64, error) {
	return s.r.FindAll(ctx, userID, lq)
}

func (s *InvoiceService) FindByID(ctx context.Context, userID uuid.UUID, invoiceID uuid.UUID) (*models.Invoice, error) {
	return s.r.FindByID(ctx, userID, invoiceID)
}

func (s *InvoiceService) Create(ctx context.Context, userID uuid.UUID, input *models.InvoiceRequest) (*models.Invoice, error) {
	return s.r.Insert(ctx, s.buildPayload(userID, input))
}

func (s *InvoiceService) Update(ctx context.Context, userID uuid.UUID, invoiceID uuid.UUID, input *models.InvoiceRequest) (*models.Invoice, error) {
...
func (s *InvoiceService) RevenueGraph(ctx context.Context, userID uuid.UUID) ([]models.RevenueGraphPoint, error) {
	now := time.Now().UTC()
	...
	// index by month key
	byMonth := make(map[string]models.RevenueGraphPoint, len(rows))
	for _, p := range rows {
		byMonth[p.Month] = p
	}

	// build full 12-month series from start to end-1month
	points := make([]models.RevenueGraphPoint, 0, 12)
	for i := 0; i < 12; i++ {
		...
		if p, ok := byMonth[key]; ok {
			...
			points = append(points, models.RevenueGraphPoint{
				Month:     key,
				Actual:    0,
				Potential: 0,
			})
...
func (s *InvoiceService) ListAttachments(ctx context.Context, userID uuid.UUID, invoiceID uuid.UUID) ([]*models.Attachment, error) {
	return s.attachmentService.FindByModelID(ctx, userID, invoiceID, s.ModelName)
}

func (s *InvoiceService) buildPayload(userID uuid.UUID, input *models.InvoiceRequest) *models.Invoice {
