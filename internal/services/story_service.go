...
func NewStoryService(repo *repositories.StoryRepository, ps *ProjectService, pus *ProjectUserService, kbs *KanbanBoardService, kcs *KanbanColumnService, as *AttachmentService) *StoryService {
...
func (s *StoryService) FindAllAssigned(ctx context.Context, userID uuid.UUID, lq *list.ListingQuery) (*list.Page[models.StoryListItem], error) {
	items, totalCount, err := s.repo.FindAllAssigned(ctx, userID, lq)
	if err != nil {
		return nil, err
	}

	return &list.Page[models.StoryListItem]{
		Items:      *items,
		Page:       lq.Page,
		PageSize:   lq.Limit,
...
func (s *StoryService) FindAll(ctx context.Context, projectID uuid.UUID, lq *list.ListingQuery) (*list.Page[models.StoryListItem], error) {
...
func (s *StoryService) FindByID(ctx context.Context, projectID, storyID uuid.UUID) (*models.Story, error) {
	return s.repo.FindByID(ctx, projectID, storyID)
}

func (s *StoryService) FindBySlug(ctx context.Context, projectID uuid.UUID, slug string) (*models.Story, error) {
	return s.repo.FindBySlug(ctx, projectID, slug)
}

func (s *StoryService) Insert(ctx context.Context, userID, projectID uuid.UUID, req *models.StoryRequest) (*models.Story, error) {
	...
	story := &models.Story{
		StoryListItem: models.StoryListItem{
			ProjectID:  projectID,
	...
	err = s.repo.WithTx(ctx, func(txRepo *repositories.StoryRepository) error {
...
func (s *StoryService) Update(ctx context.Context, projectID, storyID uuid.UUID, req *models.StoryRequest) (*models.Story, error) {
	story := &models.Story{
		StoryListItem: models.StoryListItem{
			ID:         storyID,
			ProjectID:  projectID,
...
func (s *StoryService) GetAssignee(ctx context.Context, projectID, storyID uuid.UUID) (*models.User, error) {
	return s.repo.GetAssignee(ctx, projectID, storyID)
}

func (s *StoryService) ChangeAssignee(ctx context.Context, projectID, storyID uuid.UUID, input *models.StoryAssigneeRequest) (*models.StoryAssigneeRequest, error) {
...
func (s *StoryService) ListAttachments(ctx context.Context, projectID uuid.UUID, storyID uuid.UUID) ([]*models.Attachment, error) {
	owner, err := s.projectUserService.GetOwner(ctx, projectID)
	if err != nil {
